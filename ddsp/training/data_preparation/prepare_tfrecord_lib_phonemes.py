...
def _load_audio(audio_path, sample_rate):
  ...
  alternative_audio_path=audio_path[:-4]+"_alt"+audio_path[-4:]
  logging.info("GM Loading alternative audio path '%s'.", alternative_audio_path)
...
def add_phoneme(ex, sample_rate, frame_rate):
  ...
  beam.metrics.Metrics.counter('prepare-tfrecord', 'get-phoneme').inc()
  audio = ex['audio']
  mean_loudness_db = spectral_ops_phonemes.compute_loudness(audio, sample_rate,frame_rate, 2048)
  ex = dict(ex)
